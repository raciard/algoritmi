...
typedef enum {r_ritardo_sort, r_data_sort, r_stazArr_sort,
    r_codice_sort, r_stazPart_sort,r_ricerca_bin, r_ricerca_lin, r_help, r_leggi, r_fine,r_err = -1} comando_e;

comando_e leggicomando(char comando[]);



int leggiFile(FILE* f, Corsa** out){
    int nRighe = 0;
    int i = 0;
    fscanf(f, "%d ", &nRighe);
	(*out) = malloc(sizeof(Corsa) * nRighe);
    Corsa* corse = *out;
...
int main() {
	...
	Corsa** corseByRitardo = malloc(sizeof(Corsa*) * nRighe),
		   **corseByData = malloc(sizeof(Corsa*) * nRighe),
		   **corseByStazArr  = malloc(sizeof(Corsa*) * nRighe), 
		   **corseByStazPart  = malloc(sizeof(Corsa*) * nRighe),
		   **corseByCodice = malloc(sizeof(Corsa*) * nRighe);
